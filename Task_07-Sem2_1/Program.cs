...
// вариант 1
void variant1(System.Random numberSint) // в метод передаю переменную numberSint
{
    int number = numberSint.Next(10, 100); // вызываю метод Next для определя случайного числа
    int firstNumber = number / 10;
    int secondNumber = number % 10;
    Console.WriteLine(number);

    if (firstNumber == secondNumber) Console.WriteLine("Цифры в числе одинаковые и равны - " + firstNumber);
    else if (firstNumber > secondNumber) Console.WriteLine("Максимальная цифра в цисле - " + firstNumber);
...
void variant2()
{
    int number2 = new Random().Next(10, 100); // более короткий способ определяни случайного числа
    Console.WriteLine("случайное число - " + number2);
    string stringNum = number2.ToString(); //перевожу число в строку

    if (stringNum[0] == stringNum[1]) Console.WriteLine("Цифры равны и имеют значение " + stringNum[0]);
    else if (stringNum[0] > stringNum[1]) Console.WriteLine("Максимальная цифра - " + stringNum[0]);
